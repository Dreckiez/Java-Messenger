...
public class UserGroupConversationServiceImpl implements UserGroupConversationService {
    ...
    public Map<String, Object> createGroup(Long userId, CreateGroupRequest createGroupRequest) {
        ...
        if (createGroupRequest.getUserIds().contains(userId)) {
            throw new CustomException(HttpStatus.CONFLICT, "Illegal behavior. Admin must not be included in member list.");
        }

        createGroupRequest.getUserIds().forEach(
                id -> friendRepository.findExistingFriendBetween(userId, id).orElseThrow(
                        () -> new CustomException(HttpStatus.CONFLICT, "You are no longer friends.")
                )
        );
        ...
        else groupConversation.setGroupName(createGroupRequest.getGroupName());
        ...
        GroupConversationMember admin = GroupConversationMember.builder()
                ...
                .groupConversation(entityManager.getReference(GroupConversation.class, groupConversation.getGroupConversationId()))
    ...
    public ListGroupConversationMessageResponse getMessages(Long userId, Long groupConversationId, Long cursorId) {
        ...
        GroupConversationMember groupConversationMember = groupConversationMemberRepository.findMemberInGroup(userId, groupConversationId)
                .orElseThrow(
                        () -> new CustomException(HttpStatus.FORBIDDEN, "This user is not allowed to perform this action.")
                );

        List<GroupMemberResponse> groupMemberResponseList =
                groupConversationMemberRepository.findMembersByGroupConversationId(groupConversationId);


        List<GroupConversationMessageResponse> groupConversationMessageResponseList =
            groupConversationMessageRepository.findMessages(userId, groupConversationId, cursorId);


        ListGroupConversationMessageResponse listGroupConversationMessageResponse
                = ListGroupConversationMessageResponse.builder()
    ...
    public SendMessageResponse sendMessage(Long userId,
        ...
        GroupConversation groupConversation = groupConversationRepository.findById(groupConversationId).orElseThrow(
        ...
        GroupConversationMessage groupConversationMessage = GroupConversationMessage.builder()
                ...
                .groupConversation(entityManager.getReference(GroupConversation.class, groupConversationId))
    ...
    public Map<String, String> removeMessage(Long userId, Long groupConversationId, Long groupConversationMessageId, Boolean isAll) {
        ...
        DeleteGroupConversationMessageId deleteGroupConversationMessageId
                = DeleteGroupConversationMessageId.builder()
                .memberId(userId)
    ...
    public Map<String, String> removeConversation(Long removerId, Long groupConversationId) {
        ...
        GroupConversationMember groupConversationMember = groupConversationMemberRepository.findMemberInGroup(removerId, groupConversationId)
                .orElseThrow(
                        () -> new CustomException(HttpStatus.FORBIDDEN, "This user is not allowed to perform this action.")
                );
        ...
        DeleteGroupConversation deleteGroupConversation = deleteGroupConversationRepository.findById(id).orElse(null);


        if (deleteGroupConversation == null) deleteGroupConversation = DeleteGroupConversation.builder()
                .id(id)
                .deletedAt(LocalDateTime.now())
                .build();
        else deleteGroupConversation.setDeletedAt(LocalDateTime.now());
    ...
    public Map<String, String> renameConversation(Long userId, Long groupConversationId, RenameGroupRequest renameGroupRequest) {

        GroupConversation groupConversation = groupConversationRepository.findById(groupConversationId)
                .orElseThrow(
                        () -> new CustomException(HttpStatus.NOT_FOUND, "This group no longer exists.")
                );
    ...
    public Map<String, String> updateAvatar(Long userId, Long groupConversationId, MultipartFile avatar) {
        ...
        if (avatar.isEmpty())
            ...
            try {
                String avatarUrl = cloudService.uploadGroupAvatars(avatar, groupConversation.getGroupConversationId().toString());
    ...
    public Map<String, String> addMembers(Long userId, Long groupConversationId, ModifyGroupMemberRequest modifyGroupMemberRequest) {
        GroupConversation groupConversation = groupConversationRepository.findById(groupConversationId)
                .orElseThrow(
                        () -> new CustomException(HttpStatus.NOT_FOUND, "This conversation no longer exists.")
                );
        ...
        if (alreadyIn) throw new CustomException(HttpStatus.CONFLICT, "There is any members already in the group.");
    ...
    public Map<String, String> removeMembers(Long userId, Long groupConversationId, ModifyGroupMemberRequest modifyGroupMemberRequest) {
        GroupConversation groupConversation = groupConversationRepository.findById(groupConversationId)
                .orElseThrow(
                        () -> new CustomException(HttpStatus.NOT_FOUND, "This conversation no longer exists.")
                );
        ...
        if (!allIn) throw new CustomException(HttpStatus.CONFLICT, "There is any members not in the group.");
    ...
    public Map<String, String> modifyRoleMembers(Long userId, Long groupConversationId, ModifyRoleRequest modifyRoleRequest) {
        ...
        GroupConversationMember groupConversationMember = groupConversationMemberRepository.findMemberInGroup(userId, groupConversationId)
                .orElseThrow(
                        () -> new CustomException(HttpStatus.FORBIDDEN, "You are not allowed to perform this action.")
                );
        ...
        groupConversationMember = groupConversationMemberRepository.findMemberInGroup(
                modifyRoleRequest.getMemberId(), groupConversationId
        ).orElseThrow(
                () -> new CustomException(HttpStatus.NOT_FOUND, "This user is already removed.")
        );
    ...
    public Map<String, String> out(Long userId, Long groupConversationId) {
        ...
        if (noAdmin) {
            groupConversationMember = groupConversationMemberRepository
                    ...
                    .orElse(null);
            if (groupConversationMember == null) groupConversationRepository.delete(groupConversation);
            else {
