...
public class MessageSearchDialog extends JDialog {
        ...
        new SwingWorker<List<JSONObject>, Void>() {
            ...
            protected List<JSONObject> doInBackground() throws Exception {
                ...
                if (isGlobalSearch) {
                    ...
                    urlString = API_BASE_URL + "/api/chat/user/search/" + currentChatId
                            + "/group-conversation-messages?keyword=" + encodedKeyword;
                } else {
                    // Endpoint Private cụ thể
                    urlString = API_BASE_URL + "/api/chat/user/search/" + currentChatId
                            + "/private-conversation-messages?keyword=" + encodedKeyword;
                }
            ...
            protected void done() {
                try {
                    ...
                    if (data == null || data.isEmpty()) {
                        ...
                        for (JSONObject msg : data)
                            addResultItemModern(msg);
                    }
                } catch (Exception e) {
    ...
    private void addResultItemModern(JSONObject msg) {
        ...
        if (displayContent.length() > 90)
            displayContent = displayContent.substring(0, 87) + "...";
        ...
        card.addMouseListener(new MouseAdapter() {
            public void mouseEntered(MouseEvent e) {
                card.setBackgroundColor(BG_CARD_HOVER);
                card.repaint();
            }
