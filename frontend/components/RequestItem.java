...
public class RequestItem extends BaseItem {
    ...
    public RequestItem(Request r) {
        ...
        // Giả định Avatar là component đầu tiên được BaseItem thêm vào
        // (getComponent(0))
    ...
    private void handleDeny() {
        ...
        if (response == JOptionPane.YES_OPTION) {
            sendDecision("REJECTED", "Friend request from " + request.getName() + " declined",
    ...
    private void sendDecision(String status, String successMessage, String successTitle) {
        ...
        SwingWorker<JSONObject, Void> worker = new SwingWorker<JSONObject, Void>() {
            ...
            protected JSONObject doInBackground() throws Exception {
                try {
                    JSONObject response = ApiClient.postJSON(ApiUrl.DECIDE_FRIEND_REQUEST, payload,
                            UserSession.getUser().getToken());
            ...
            protected void done() {
                try {
                    ...
                    if (response != null && response.optInt("httpStatus", 200) == 200) {
                        ...
                        String errorMsg = (response != null)
                                ? response.optString("message", "Failed to process friend request")
